...
class baseClass:
	...
	def stitchTT(self, name, madHT, nEle, nMuo, nTau, GenMET, fileID):
		...
		if "18" in fileID:# 2018 doesnt have GenMET samples, so we dont do the GenMET cuts for this year
			if "TTJets_Tune" in name:
				if not (madHT < 600 and nEle==0 and nMuo==0 and nTau==0): 
					return True
				else:
					return False
			elif "TTJets_HT" in name:
				if not (madHT >= 600):
					return True
				else:
					return False
			elif "TTJets_DiLept" in name or "TTJets_SingleLeptFromT" in name or "TTJets_DiLept" in name:
				if not (madHT < 600):
					return True
				else:
					return False
			...
			if "TTJets_Tune" in name:
				...
				if not (madHT < 600 and GenMET < 150):
					...
					return False
			elif "TTJets_DiLept_genMET" in name or "TTJets_SingleLeptFromT_genMET" in name or "TTJets_DiLept_genMET" in name:
				if not (madHT < 600 and GenMET >= 150):
					return True
				else:
					return False
